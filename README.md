...
This program compresses and decompresses files using Huffman coding. 
It counts how often each character appears, builds a Huffman tree, 
and gives each character a code of 0s and 1s. During compression, 
it writes a header with character frequencies and replaces each 
character with its code. During decompression, it reads the header,
rebuilds the tree, and converts the bits back into characters. 
It also handles empty files and files with only one character.
The program uses a priority queue to join the smallest frequencies first,
ensuring optimal compression. It also frees all allocated memory after use
to prevent leaks and ensure efficiency.
